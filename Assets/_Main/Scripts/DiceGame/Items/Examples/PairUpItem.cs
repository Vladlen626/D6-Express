...
{
	/// <summary>
	/// Arms on click: select N dice; all selected dice become the highest face among them.
	/// </summary>
	public class PairUpItem : ModifierItemBase, IOnPassModifier, IOnRoundStartModifier, IModifierItemViewProvider
	{
			...
			: base(id, id, DiceItemActivationType.ClickToActivate)
		{
			...
			selectionTarget = Mathf.Max(2, selectionCount);
		...
		private async UniTask ApplyPairUpAsync()
		{
			...
			try
			{
				...
				var max = targets[0].CurrentValue;
				for (int i = 1; i < targets.Count; i++)
				{
					if (targets[i].CurrentValue > max)
					{
						max = targets[i].CurrentValue;
					}
				}

				for (int i = 0; i < targets.Count; i++)
				{
					targets[i].SetValue(max);
				}
