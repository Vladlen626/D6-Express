...
{
	/// <summary>
	/// Arms on click: next clicked unsaved and unbanked die flips to opposite face, then item is consumed.
	/// </summary>
	public class SelectedToOppositeItem : ModifierItemBase, IOnPassModifier, IOnRoundStartModifier, IModifierItemViewProvider
	{
		...
		public SelectedToOppositeItem(string id, DiceScoringService scoringService, ItemView prefabOverride = null)
		...
		private void OnDiceClicked(DiceModel model)
		{
			...
			model.SetValue(GetOppositeValue(model.CurrentValue));
			UpdatePreview();
