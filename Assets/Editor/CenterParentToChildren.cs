...
public static class CenterParentOnChildren
{
    ...
    private static void CenterOne(Transform parent)
    {
        ...
        Bounds b = renderers.Length > 0 
            ? renderers.Aggregate(new Bounds(renderers[0].bounds.center, Vector3.zero), (bb, r) => { bb.Encapsulate(r.bounds); return bb; })
            : children.Aggregate(new Bounds(children[0].position, Vector3.zero), (bb, c) => { bb.Encapsulate(c.position); return bb; });
